...
public class TestReport implements IReporter, ISuiteListener {
    ...
    private void createReport(File file) throws IOException {
        ...
        for (ISuite suite : testSuites) {
            ...
            for (ISuiteResult result : suiteResult.values()) {
                ...
                generateSkippedTable(result.getTestContext().getSkippedTests(),
    ...
    private List<String> getAllSkippedMethodNames() {
        List<String> skippedMethodNames = new ArrayList<>();
        for (ISuite suite : testSuites) {
            Map<String, ISuiteResult> suiteResult = suite.getResults();
            for (ISuiteResult result : suiteResult.values()) {
    ...
    private List<String> getAllPassedMethodNames() {
        List<String> passedMethodNames = new ArrayList<>();
        for (ISuite suite : testSuites) {
            Map<String, ISuiteResult> suiteResult = suite.getResults();
            for (ISuiteResult result : suiteResult.values()) {
                ITestContext context = result.getTestContext();
        ...
        return passedMethodNames;
    }

    private List<String> getAllFailedMethodNames() {
    ...
    private void generateGraph() {
        ...
        for (ISuite suite : this.testSuites) {
            ...
            for (ISuiteResult sResult : suiteResults.values()) {
                ...
                List<String> skippedMethodNames = getAllSkippedMethodNames();
                List<String> failedMethodNames = getAllFailedMethodNames();
                List<String> passedMethodNames = getAllPassedMethodNames();

                if (skippedMethodNames.isEmpty()) {
                    break;
                }

                for (String skippedMethodName : skippedMethodNames) {
                    if (!failedMethodNames.contains(skippedMethodName) && !passedMethodNames.contains(skippedMethodName)) {
                        skipped += 1;
                    }
                }
            }
    ...
    private void addDuplicateTrackingWarning(ITestResult result, final BiMap<String, String> test, final List<Pair<String, String>> warnings) {
    ...
    private void addRallyReportingWarning(ITestResult result, final List<Pair<String, String>> warnings) {
        ...
        if (warn != null) {
            warnings.add(Pair.of(String.format("%s.%s",
                    result.getMethod().getTestClass().getName(),
                    result.getMethod().getMethodName()), (String) warn));
        }
    }
    ...
    private void generateSkippedTable(IResultMap skippedTestMap,
    ...
    private String getSimilarTestCaseID(ITestNGMethod testMethod) {
        ...
        Tracking annotation = testMethod.getConstructorOrMethod().getMethod().getAnnotation(Tracking.class);
        if (annotation != null) {
            result = annotation.similarTestID().isEmpty() ? "----" : annotation.similarTestID();
        }
        return result;
    }
